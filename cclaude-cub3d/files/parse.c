...
// cub 파일 한줄에 있는 요소들을 확인해서 처리해 주는 함수
int		ft_line(t_all *s, char *line)
{
	...
	// 해상도
		...
		s->err.n = ft_res(s, line, &i);		// 해상도 파싱해오기
	// 북 이미지
	...
	if (ft_spaceskip(line, &i) && s->err.n == 0 && line[i] != '\0')
		return (ft_strerror(-10));  // "Error : Invalid line in file"
...
int		ft_parse(t_all *s, char *cub)
{
	...
	if (fd == -1)
		return (ft_strerror(-1));  // "Error : Couldn't open file (FD)"
	while (ret == 1)
	{
